...
if __name__ == "__main__":
    ...
    remote_config_script = f"{os.path.split(args.config_script)[-1]}.distributed"
    for it, addr in enumerate(all_addresses):
        ...
        # )
        tmux_command = wrap_single(
            f"tmux new-session -s {tmux_name} -d {bash_command}"
        )

        ssh_command = f"{args.ssh_cmd.format(addr=addr)} {tmux_command}"
