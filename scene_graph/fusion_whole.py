...
class Same_Node_Detection(object):
    ...
    def compare_position(self, c_m, c_v, p_m, p_v, prev_pt_num, new_pt_num):
        ...
        distance = c_m - p_m
        I_x = 1.0 if (
            np.abs(distance[0]) < self.th
        ) else self.th / (np.abs(distance[0]))
        I_y = 1.0 if (
            np.abs(distance[1]) < self.th
        ) else self.th / (np.abs(distance[1]))
        I_z = 1.0 if (
            np.abs(distance[2]) < self.th
        ) else self.th / (np.abs(distance[2]))
    ...
    def compare_color(self, curr_hist, prev_hist):
        ...
        dist = np.sqrt(
            np.sum(np.power(np.subtract(curr_rgb, prev_rgb), 2))) / (255 * np.sqrt(3)
        )
        score = 1 - dist
        return score
    ...
    def node_update(self, global_node, curr_mean, curr_var, curr_pt_num, curr_cls, cls_score, curr_color_hist):
        ...
        if node_score > self.node_th:
            print("updated node_score and id --> class name: {},{}, {}".format(
                node_score,
                max_score_index,
                global_node[str(matched_idx)]['class'])
            )
            print("cls_score and pos_score: {},{}".format(
                cls_test[max_score_index], pos_test[max_score_index])
            )
        else:
...
class TSDFVolume:
    ...
    def __init__(self, vol_bnds, voxel_size, use_gpu=True, root_path=None, cfg=None):
        ...
        self.cluster_object = ['dining table', 'bench',
                               'bottle', 'cup', 'chair', 'bed', 'couch']
        self.mask_data = np.ones([])

        # Define voxel volume parameters
        self._vol_bnds = vol_bnds
        self._voxel_size = float(voxel_size)
        # truncation on SDF (orig default : 5)
        self._trunc_margin = 3*self._voxel_size
        ...
        if self.gpu_mode:
            ...
            self._cuda_src_mod = SourceModule("""
                ...
                __global__ void integrate(float * tsdf_vol,
                    ...
                    int pixel_x = (int) roundf((cam_pt_x/cam_pt_z)*other_params[3]/2/tan(other_params[11]/2*PI/180.0)+other_params[3]/2-0.5);
                    int pixel_y = (int) roundf((cam_pt_y/cam_pt_z)*other_params[2]/2/tan(other_params[11]/2*PI/180.0)+other_params[2]/2-0.5);
    ...
    def update(self, vol_bnds, prev_vol_bnds, vol_min, vol_max):
        ...
        if (x_extend_b > 0):
            ...
            self._vol_bnds[:, 0] = self._prev_vol_bnds[:, 0] - \
                np.array([x_extend_b*self._voxel_size, 0., 0.])
            self._vol_origin[0] = self._prev_vol_bnds[:, 0][0] - \
                x_extend_b*self._voxel_size
            self._vol_bnds = np.add(self._vol_bnds, np.array(
                [x_extend_b * self._voxel_size, 0., 0.]).reshape(3, 1))
        ...
        if (y_extend_b > 0):
            ...
            self._vol_bnds[:, 0] = self._prev_vol_bnds[:, 0] - \
                np.array([0., y_extend_b * self._voxel_size, 0.])
            self._vol_origin[1] = self._prev_vol_bnds[:, 0][1] - \
                y_extend_b * self._voxel_size
            self._vol_bnds = np.add(self._vol_bnds, np.array(
                [0., y_extend_b*self._voxel_size, 0.]).reshape(3, 1))
        ...
        if (z_extend_b > 0):
            ...
            self._vol_bnds[:, 0] = self._prev_vol_bnds[:, 0] - \
                np.array([0., 0., z_extend_b * self._voxel_size])
            self._vol_origin[2] = self._prev_vol_bnds[:, 0][2] - \
                z_extend_b * self._voxel_size
        ...
        if (x_extend_u > 0):
            ...
            for i in range(x_extend_u):
                self._tsdf_vol_cpu = np.insert(
                    self._tsdf_vol_cpu, self._tsdf_vol_cpu.shape[0], 1, axis=0)
                self._weight_vol_cpu = np.insert(
                    self._weight_vol_cpu, self._weight_vol_cpu.shape[0], 0, axis=0)
                self._color_vol_cpu = np.insert(
                    self._color_vol_cpu, self._color_vol_cpu.shape[0], 0, axis=0)
                self._class_vol_cpu = np.insert(
                    self._class_vol_cpu, self._class_vol_cpu.shape[0], -1, axis=0)
        if (y_extend_u > 0):
            ...
            for j in range(y_extend_u):
                self._tsdf_vol_cpu = np.insert(
                    self._tsdf_vol_cpu, self._tsdf_vol_cpu.shape[1], 1, axis=1)
                self._weight_vol_cpu = np.insert(
                    self._weight_vol_cpu, self._weight_vol_cpu.shape[1], 0, axis=1)
                self._color_vol_cpu = np.insert(
                    self._color_vol_cpu, self._color_vol_cpu.shape[1], 0, axis=1)
                self._class_vol_cpu = np.insert(
                    self._class_vol_cpu, self._class_vol_cpu.shape[1], -1, axis=1)
        if (z_extend_u > 0):
            ...
            for k in range(z_extend_u):
                self._tsdf_vol_cpu = np.insert(
                    self._tsdf_vol_cpu, self._tsdf_vol_cpu.shape[2], 1, axis=2)
                self._weight_vol_cpu = np.insert(
                    self._weight_vol_cpu, self._weight_vol_cpu.shape[2], 0, axis=2)
                self._color_vol_cpu = np.insert(
                    self._color_vol_cpu, self._color_vol_cpu.shape[2], 0, axis=2)
                self._class_vol_cpu = np.insert(
                    self._class_vol_cpu, self._class_vol_cpu.shape[2], -1, axis=2)
    ...
    def integrate(self, color_im, depth_im, fov, cam_pose,
                boxes,
        ...
        comp_ID = []
        if num_masks > 0:
            self.masks_test = masks.squeeze(-1).to(torch.device("cpu")
                                                    ).detach().numpy().astype(np.float32)
            self.mask_data = - \
                np.ones(self.masks_test[0].shape, dtype="int32").reshape(-1)
            self.seen_class = {}
            for i in range(num_masks):
                class_value = self.class_info[i]
                class_name = class_value.split(':')[0]
                class_index = int(class_value.split(':')[1].split('_')[0])
                class_score = int(float(class_value.split(':')[1].split('_')[
                                1])*100)-1   # set value to 0 ~ 99
                ...
                if (self.is_depth_clustered):
                    ...
                    self.depth_val[self.masks_test[i].reshape(
                        -1).nonzero()] = depth_im[self.masks_test[i].nonzero()]
                    self.check_depth = self.depth_val[self.depth_val.nonzero(
                    )].reshape(-1, 1)

                    if (self.cluster_technique == 'kmeans'):
                        self.cluster = KMeans(
                            n_clusters=3, random_state=0).fit(self.check_depth)

                    if (self.cluster_technique == 'meanshift'):
                        bandwidth = estimate_bandwidth(
                            self.check_depth, quantile=0.2, n_samples=500)
                        self.cluster = MeanShift(
                            bandwidth=bandwidth, bin_seeding=True).fit(self.check_depth)
                    ...
                    if (self.cluster_technique == 'ransac'):
                        ...
                        self.clustered_index = np.zeros(
                            self.masks_test[0].reshape(-1).shape).astype('int')
                        self.clustered_index[self.depth_val.nonzero()
                                            ] = inliers.astype('int')
                    ...
                    if (self.cluster_technique == 'kmeans' or self.cluster_technique == 'meanshift'):
                        self.clustered_index = - \
                            np.ones(self.masks_test[0].reshape(-1).shape).astype('int')
                        self.clustered_index[self.depth_val.nonzero()
                                            ] = self.cluster.labels_
                        self.center_label = np.argsort(self.cluster.cluster_centers_, axis=0)[
                            0][0]  # get nearest label
                        self.valid_pts_index = self.clustered_index == self.center_label

                # get color hists and mask info for 3D reconstruction
                if (first_masking):
                    if (self.is_depth_clustered):
                        color_pixs = self.color_im[self.valid_pts_index.reshape(
                            self.masks_test[0].shape)]
                        if color_pixs.shape[0] != 0:
                            self.ID_2D += 1
                            ...
                            self.make_node_data(self.ID_2D, class_name,
                                                boxes[i, :], self.color_hist, is_new=True)
                            comp_ID += [self.ID_2D]
                        else:
                    ...
                    update_id = max(self.unique_ID_3D, default=0) + i + 1
                    ...
                    if (self.is_depth_clustered):
                        ...
                        if color_pixs.shape[0] != 0:
                            ...
                            self.make_node_data(update_id, class_name,
                                                boxes[i, :], self.color_hist, is_new=True)
        ...
        if self.gpu_mode:  # GPU mode: integrate voxel volume (calls CUDA kernel)
            ...
            self.cuda_integrate_param(
                obs_weight, num_masks, update_masked_img, update_tsdf, first_masking, color_im, depth_im)
    ...
    def widen_camera_view(self, voxel, type):
        ...
        if (type == 'color'):
            ...
            class_vol_ids = (class_view).astype('int') - \
                ((class_view / 100).astype('int') * 100)
            class_vol_label = (class_view/100).astype('int') - \
                (class_view / 10000).astype('int') * 100
            class_vol_score = (class_view/10000).astype('int') - \
                (class_view / 1000000).astype('int') * 100
            ...
            if (is_bounding_box_type):
                for (idx, num) in freq_id:
                    ...
                    min_y, max_y = min(pix_y[valid_pix][idx_loc]), max(
                        pix_y[valid_pix][idx_loc])
                    min_x, max_x = min(pix_x[valid_pix][idx_loc]), max(
                        pix_x[valid_pix][idx_loc])
    ...
    def draw_updated_3d_pts(self):
        ...
        nonzero_class_vol = self._class_vol_cpu[(
            self._class_vol_cpu + 1).nonzero()].copy()
        ID_3D = nonzero_class_vol.astype(
            'int') - (nonzero_class_vol / 100).astype('int') * 100
        ...
        if self.debug_same_node_detector:
            ...
            self.class_label = []
            self.bbox_3ds = {}
            class_vol_ids = (self._class_vol_cpu).astype('int') - \
                ((self._class_vol_cpu / 100).astype('int') * 100)
            detect_cnt = (self._class_vol_cpu / 1000000).astype('int')
            for idx in unique_ID_3D:
                ID_index = np.where(np.logical_and(
                    class_vol_ids == idx, detect_cnt >= 1))
                # ID_index = np.where(class_vol_ids == idx)
                self.ID_index_array = np.array(ID_index)
                print(self.ID_index_array)
                if (len(ID_index[0]) != 0):
                    ID_3D_pose = self.ID_index_array.transpose() * self._voxel_size + \
                        self._vol_origin.reshape(-1, 3)
                    ID_3D_mean = np.mean(ID_3D_pose, axis=0)
                    ID_class = int(self._class_vol_cpu[ID_index][0] / 100) - \
                        int(self._class_vol_cpu[ID_index][0] / 10000) * 100
            ...
            # print(f'self.camera_3D_pose: {self.camera_3D_pose}')
            ...
            self.cam_connect = np.array([[0, 1], [0, 2], [0, 3], [0, 4], [1, 2], [
                                        2, 3], [3, 4], [4, 1]], dtype=np.int32)
    ...
    def draw_scene_graph(self, frame_num):
        ...
        sg.format = 'png'
        # sg.size = "480,640"
        sg.render(os.path.join(self.scene_graph_path,
                'scene_graph' + str(frame_num)), view=False)
        self.f_idx += 1
