...
import cv2
import os
from orb_setting import fusion_whole
...
class Scan:
  ...
  def __init__(self, rgb_paths=None, depth_paths=None, pose_paths=None, cam_intr=None, mesh_plot=True,
               ...
               apply_orb_slam=True, slam=None, skip_im=15):
    ...
    # orb_slam settings
    self.apply_orb_slam = apply_orb_slam
    self.slam = slam
  ...
  def get_orbslam2_cam_pose(self, trajectory, idx):
      stamp, r00, r01, r02, t0, r10, r11, r12, t1, r20, r21, r22, t2 = trajectory[idx]
      cam_pose = np.array([[r00, r01, r02, t0],
                           [r10, r11, r12, t1],
                           [r20, r21, r22, t2],
  ...
  def open_scan(self, rgb_names, depth_names, pose_names, frame_num, recon=True):
    ...
    if self.apply_orb_slam:
        print('frame_num:{}'.format(frame_num))
        self.slam.process_image_rgbd(color_image.copy(), depth_im.copy(), frame_num*0.032)
        self.trajectory = self.slam.get_trajectory_points()
        cam_pose = self.get_orbslam2_cam_pose(self.trajectory, frame_num)
    ...
    if (frame_num % self.skip_im == 0):
        self.color_dict[frame_num] = color_image
        self.depth_dict[frame_num] = depth_im
        self.yolact_dict[frame_num] = {}
        self.yolact_dict[frame_num]['boxes'] = self.boxes
        self.yolact_dict[frame_num]['masks'] = self.masks
        self.yolact_dict[frame_num]['num_dets_to_consider'] = self.num_dets_to_consider
        self.yolact_dict[frame_num]['text_str'] = self.text_str
        # self.yolact_dict[frame_num]['masked_img'] = self.masked_img
    ...
    if (recon):
        ...
        self.vol_bnds_created = False
        for fidx in range(0, frame_num+1, self.skip_im):
            color_image = self.color_dict[fidx]
            depth_im = self.depth_dict[fidx]
            cam_pose = self.get_orbslam2_cam_pose(self.trajectory, fidx)
            boxes = self.yolact_dict[fidx]['boxes']
            masks = self.yolact_dict[fidx]['masks']
            num_dets_to_consider = self.yolact_dict[fidx]['num_dets_to_consider']
            text_str = self.yolact_dict[fidx]['text_str']
            ...
            if (self.vol_bnds_created):
              ...
              # voxel_size : 0.04
              self.tsdf_vol = fusion_whole.TSDFVolume(self.vol_bnds, voxel_size=0.06, use_gpu=self.use_gpu,
            ...
            self.tsdf_vol.integrate(color_image, depth_im, self.cam_intr, cam_pose,
                                    boxes, masks, num_dets_to_consider, text_str,
