    ...
    def update_3d_recon(self):
        ...
        if (self.offset % self.skip_im == 0):
            ...
            if self.mesh_plot:
                ...
                point_cloud = self.scan.open_scan(self.rgb_names[self.offset],
                                                  self.depth_names[self.offset],
                                                  self.pose_names[self.offset],
                                                  self.offset,
                                                  recon=True
            ...
            if (self.view_point_cloud):
                ...
                if self.mesh_plot:
                    ...
                    if self.scan.num_dets_to_consider > 0 and self.scan.tsdf_vol.debug_same_node_detector:
                        ...
                        self.label_vis.font_size = int(self.scan.tsdf_vol._voxel_size * 500)
                    ...
                    point = np.vstack((point_cloud[:, :3], np.array([[self.scan.tsdf_vol.mean_x, self.scan.tsdf_vol.mean_y, self.scan.tsdf_vol.mean_z]])))
                    color = np.vstack((point_cloud[:, 3:]/255., np.array([[255., 0., 0.]])/255.))
                    self.scan_vis.set_data(point_cloud[:, :3],
                                           face_color=point_cloud[:, 3:]/255.,
                                           edge_color=point_cloud[:, 3:]/255.,
                                           size=1)
            ...
            if not self.scan.use_gpu:
                self.depth_mask_vis.set_data(self.scan.tsdf_vol.masked_img)
                self.depth_mask_vis.update()

                scene_graph_path = os.path.join(self.scan.tsdf_vol.root_path, 'scene_graph',
                                                'scene_graph' + str(self.scan.tsdf_vol.f_idx - 1) + '.png')
                if (self.scan.tsdf_vol.f_idx > 0):
                    scene_graph = cv2.cvtColor(cv2.imread(scene_graph_path), cv2.COLOR_BGR2RGB)
                    scene_graph = cv2.resize(scene_graph, (640, 480), interpolation=cv2.INTER_AREA)
                    self.scene_graph_vis.set_data(scene_graph)
                    self.scene_graph_vis.update()
