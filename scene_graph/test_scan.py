...
from . import fusion_whole
from .utils.augmentations import BaseTransform, FastBaseTransform, Resize
from .utils.util import get_camera_pose, get_view_frustum
...
class Scan:
    ...
    def open_scan(
        ...
        camera_locs: np.ndarray,    # (cam_x, cam_y, cam_z, cam_rot, cam_horizon)
        frame_num: int, 
        recon: bool = True,
    ):
        ...
        if cam_pose.size != 0:  # update if orb_py has cam_pose
            if (recon):
                ...
                if (self.vol_bnds_created):
                    ...
                    # voxel_size : 0.04
                    self.tsdf_vol = fusion_whole.TSDFVolume(self.vol_bnds, voxel_size=0.02, use_gpu=self.use_gpu,
                ...
                if (self.num_dets_to_consider > 0):
                    self.prev_color_image = rgb_image.copy()
                ...
                if (self.masked_img_number <= 1):
                    ...
                    first_masking = 0
                self.tsdf_vol.integrate(rgb_image, depth_image, fov, cam_pose,
                                            self.boxes,
