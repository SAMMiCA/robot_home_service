...
class HomeServiceTaskSpec:
    ...
    def __init__(
        ...
        agent_rotations: Optional[float] = None,
    ...
    def unique_id(self):
        ...
        return f"{self.scene}__{self.stage}__{self.pickup_object}__{self.start_receptacle}__{self.place_receptacle}"
...
class HomeServiceTHOREnvironment:
    ...
    def __init__(
        ...
        self.object_name_to_target_pose: Optional[Dict[str, Dict]] = None
    ...
    def drop_held_object_with_snap(self) -> bool:
        ...
        with include_object_data(self.controller):
            ...
            goal_pose = self.object_name_to_target_pose[held_obj["name"]]
    ...
    def poses(
        ...
        * `current_poses[i]` - corresponds to the pose of the ith object in the current environment.
        ...
        ordered_obj_names = list(self.object_name_to_target_pose.keys())
        ...
        for obj_name in ordered_obj_names:
            if obj_name not in obj_name_to_current_obj:
                ...
                if not self._have_warned_about_mismatch:
                    ...
                    ssos = set(self.object_name_to_start_pose.keys())
                    tsos = set(self.object_name_to_target_pose.keys())
                ...
                obj_name_to_current_obj[obj_name] = {
                    **self.object_name_to_target_pose[obj_name],
                    "isBroken": True,
                    "broken": True,
                    "position": None,
                    "rotation": None,
        ...
        if self._sorted_and_extracted_target_poses is None:
            broken_obj_names = [
                obj_name
                for obj_name in ordered_obj_names
                if self.object_name_to_target_pose[obj_name]["isBroken"]
            ]
            if len(broken_obj_names) != 0:
                ...
                for broken_obj_name in broken_obj_names:
                    self.object_name_to_target_pose[broken_obj_name][
                        "isBroken"
                    ] = False
                    self.object_name_to_start_pose[broken_obj_name][
                        "isBroken"
                    ] = False
                ordered_obj_names = list(self.object_name_to_target_pose.keys())

            target_poses = tuple(
                self.object_name_to_target_pose[k] for k in ordered_obj_names
            )
            start_poses = tuple(
                self.object_name_to_start_pose[k] for k in ordered_obj_names
            )
    ...
    def _task_spec_reset(
        ...
        if force_axis_aligned_start:
            self.current_task_spec.agent_rotations = round_to_factor(
                self.current_task_spec.agent_rotations, 90
            )

        pos = self.current_task_spec.agent_positions
        rot = {
            "x": 0, 
            "y": self.current_task_spec.agent_rotations,
            "z": 0,
        }
        self.controller.step(
            "TeleportFull",
        ...
        # Remove objects for test (should not be seen in training environments)
...
class HomeServiceEnvironment:
    ...
    def poses(
        ...
        * `current_poses[i]` - corresponds to the pose of the ith object in the current environment.
        ...
        ordered_obj_ids = list(self.object_name_to_target_pose.keys())
    ...
    def horizontal_to_vertical_fov(
        ...
        return HomeServiceProcTHOREnvironment.vertical_to_horizontal_fov(
    ...
    def _get_intrinsics(h, w, fv_deg):
        fh = (
            HomeServiceProcTHOREnvironment.vertical_to_horizontal_fov(fv_deg, h, w)
            * math.pi
            / 180
        )
    ...
    def find_short_path(
        ...
        for attempt in range(num_attempts):
            ...
            new_path = HomeServiceProcTHOREnvironment.strip_return_from_cycle(path)
    ...
    def _differences(self, poses1, poses2):
        ...
        for gp, cp in zip(poses1, poses2):
            ...
            ) and not HomeServiceProcTHOREnvironment.are_poses_equal(gp, cp):
