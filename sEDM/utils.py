...
def fuzzy_and(x, y):
    f_and = np.zeros(x.size)
    for i in range(f_and.size):
        if x[i] > y[i]:
            f_and[i] = y[i]
        else:
            f_and[i] = x[i]
...
def fuzzy_or(x, y):
    f_or = np.zeros(x.size)
    for i in range(f_or.size):
        if x[i] < y[i]:
            f_or[i] = y[i]
        else:
            f_or[i] = x[i]
...
def err(input, weight):
    w1 = weight[:len(weight)//2]
    w2 = weight[len(weight)//2:]
    e = np.linalg.norm(fuzzy_and(input,w1)-w1 + fuzzy_or(input,w2)-w2 ,2)
    return e
