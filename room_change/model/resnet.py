...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        if opt.mode == 'label':
            self.fc = nn.ModuleList([NormedLinear(512 * block.expansion, num_classes)])
            self.fc = nn.Linear(512 * block.expansion, num_classes, bias=False)
        elif opt.mode == 'self':
            prev_dim = 512 * block.expansion
            dim = 512
            pred_dim = 512
    ...
    def _forward_impl(self, x):
        ...
        output = self.scale_factor * self.fc(out)

        return out, output

    def _forward_impl_self(self, x):
        # See note [TorchScript super()]
    ...
    def forward(self, x):
        if self.opt.mode == 'label':
            return self._forward_impl(x)
        elif self.opt.mode == 'self':
            return self._forward_impl_self(x)
